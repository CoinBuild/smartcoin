...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Smartcoin address (e.g. %1)").arg("SWpVXKuQiGt8NdgXAvBWs5CSPdBAEmMGPm"));
#endif
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::SMC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SMC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
boost::filesystem::path static StartupShortcutPath()
{
    if (GetBoolArg("-testnet", false))
        ...
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Smartcoin (regtest).lnk";
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (GetBoolArg("-testnet", false))
            optionFile << "Name=Smartcoin (testnet)\n";
        else if (GetBoolArg("-regtest", false))
            optionFile << "Name=Smartcoin (regtest)\n";
        else
